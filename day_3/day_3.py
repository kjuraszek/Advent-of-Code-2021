...
class DiagnosticTool():
    ...
    def get_common_values(self, diagnostic_data, index):
    ...
    def get_common_bits(self):
        for index in range(self.diagnostic_data_width):
            most_common = self.get_common_values(self.diagnostic_data, index)
    ...
    def calculate_rating_by_bit(self, data, position, bit, common):
        if len(data) == 1:
            ...
            temp_data = []
            most_common_bits = self.get_common_values(data, position)
